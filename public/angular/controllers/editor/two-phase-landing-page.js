/* eslint-disable */
controllers.controller('editorTwoPhaseLPController', function($scope, $rootScope, $http, $window, $location, Utils, editorCompiler, uploader) {


  $scope.CURRENT_TEMPLATE = 'TWO-PHASE-LANDING-PAGE';
  var id = 'hotspot';
  var container = 'hotspotContainer';
  ...
  $scope.renderHotspot = function() {
    ...
    $(function() {
      $('#' + id).remove();
      var iframe = $('<iframe frameborder="0" name="hotspot" id="' + id + '" class="renderIframe" scrolling="no"> </iframe>');
  ...
  $scope.$on('readyToRecieveLandingPageValues', function (event, data) {
    ...
    $rootScope.$broadcast('splashScreenValues', values);
  });

  $scope.$on('uploadFile', function(event, data) {
    uploadFiles('advertisement/' + data.id + '/TWO-PHASE-LANDING-PAGE/', data.progressBarEvent);
  });
